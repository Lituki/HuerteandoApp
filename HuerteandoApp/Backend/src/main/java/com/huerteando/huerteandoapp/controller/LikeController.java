...
import com.huerteando.huerteandoapp.service.IObservacionLikeService;
...
@RestController
@RequestMapping("/api/observaciones/{idObservacion}/likes")
public class LikeController {

    private final IObservacionLikeService likeService;

    public LikeController(IObservacionLikeService likeService) {
        this.likeService = likeService;
    }

    // GET /api/observaciones/{idObservacion}/likes/count
    ...
    // Devuelve un Map para que el JSON sea: { "likes": 42 }
    @GetMapping("/count")
    public ResponseEntity<Map<String, Long>> contar(@PathVariable Long idObservacion) {
        ...
        return ResponseEntity.ok(Map.of("likes", total));
    }

    // GET /api/observaciones/{idObservacion}/likes/existe?idUsuario=1
    ...
    // Devuelve: { "yaLikeado": true } o { "yaLikeado": false }
    @GetMapping("/existe")
    public ResponseEntity<Map<String, Boolean>> existe(
        ...
        return ResponseEntity.ok(Map.of("yaLikeado", existe));
    }

    // POST /api/observaciones/{idObservacion}/likes?idUsuario=1
    ...
    // DELETE /api/observaciones/{idObservacion}/likes?idUsuario=1
